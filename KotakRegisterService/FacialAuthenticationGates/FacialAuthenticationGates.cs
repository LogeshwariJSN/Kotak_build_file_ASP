...
namespace KMBL.StepupAuthentication.CoreComponents
{
    ...
    public class get_registered_user_data
    {
        ...
        public string customer_image2_id { get; set; }
        public string customer_image3_id { get; set; }
    ...
    public class ExecutingSP
    {
        ...
        public static string MongoDB_Registration_Databse = ConfigurationManager.AppSettings["MongoDB_Registration_Databse"];
        ...
        public List<get_registered_user_data> GetRegisteredUserData(int offset_value, int record_limit, string resource_group_id)
        {
            ...
            parameters.Add(dbManager.CreateParameter("v_resource_group_id", record_limit, OracleDbType.Varchar2));
            parameters.Add(dbManager.CreateParameter("result_cursor", OracleDbType.RefCursor, ParameterDirection.Output));
            OracleCommand objCmd = dbManager.GetCommand("SP_GET_REGISTERED_USER_DATA", CommandType.StoredProcedure, parameters.ToArray(), out connection);
            ...
            while (reader.Read())
            {
                ...
                grudi.customer_image_id = reader.IsDBNull(2) ? null : Convert.ToBase64String(ImageGallery.Get(reader.GetString(2)));
                    grudi.customer_image2_id = reader.IsDBNull(3) ? null : reader.GetString(3);
                grudi.customer_image3_id = reader.IsDBNull(4) ? null : reader.GetString(4);
        ...
        public int update_resource_group_id(string resource_group_id, string customer_registration_checker_id)
        {
            var dbManager = new DBManager("DBConnection");
            ...
            parameters.Add(dbManager.CreateParameter("v_customer_registration_checker_id", customer_registration_checker_id, OracleDbType.NVarchar2));
            parameters.Add(dbManager.CreateParameter("result_cursor", OracleDbType.RefCursor, ParameterDirection.Output));
            OracleCommand objCmd = dbManager.GetCommand("SP_UPDATE_RESOURCE_GROUP_ID", CommandType.StoredProcedure, parameters.ToArray(), out connection);

            connection.Open();
            objCmd.ExecuteNonQuery();

            int Status = int.Parse(objCmd.Parameters["result_cursor"].Value.ToString());
