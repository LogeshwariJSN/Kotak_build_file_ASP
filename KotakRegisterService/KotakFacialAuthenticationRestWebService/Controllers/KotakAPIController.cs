...
namespace KotakFacialAuthenticationRestWebService.Controllers
{
    public class KotakAPIController : ApiController
    {
        ...
        private static string DBConnection = ConfigurationManager.ConnectionStrings["DBConnection"].ConnectionString;
        ...
        private byte[] SurveySecretKey = Encoding.UTF8.GetBytes(ConfigurationManager.AppSettings["SurveyAPIKey"]);
        ...
        [HttpPost]
        [Route("Survey/GetSurveyQuestionAndAnswer")]
        public object GetSurveyQuestionAndAnswer()
        {
            try
            {
                ExecutingSP es = new ExecutingSP();
                sp_register_data_logs("", 0, 0, 0, "", "", "Calling Survey Question and Answer API");
                survey_question_and_answer sqaa = es.survey_question_and_answer();
                var json = JsonConvert.SerializeObject(new
                {
                    operations = sqaa
                });
                sp_register_data_logs("", 0, 0, 0, "", json, "Response Survey Question and Answer API");
                return sqaa;
            }
            catch(Exception e)
            {
                new ExecutingSP().SP_Exception("", 0, 0, "", e.Message, null, e.Source, e.StackTrace, e.TargetSite.ToString());
                return ExceptionAccured();
            }
        }

        [HttpPost]
        [Route("Survey/GetSurveyUserAnswer")]
        public object GetSurveyUserAnswer([FromBody] JObject json)
        {
            ...
            try
            {
                ...
                string CRN = null;
                    ...
                    if (headers.Contains("CRN") && headers.Contains("SequenceId") && headers.Contains("Authorization"))
                    {
                        CRN = headers.GetValues("CRN").First();
                        string SequenceId = headers.GetValues("SequenceId").First();
                        string Authorization = headers.GetValues("Authorization").First();

                        if (InputValidate(SequenceId) || SequenceId.Length != 32 || InputValidate(Authorization))
                            throw new InsufficientHeaderDetails("SURVEY100", "Insufficient Header Details in Survey Validatation (CRN, SequenceId and Authorization)");

                        if (SurveyAnswerRequestHashing(CRN, SequenceId, Authorization))
                        {
                            ExecutingSP es = new ExecutingSP();
                            string get_user_survey_id = es.InsertUserSurveyData(json["CRN"].ToString(), Convert.ToInt32(json["customer_registration_checker_id"]), Convert.ToInt32(json["rate_experience"]), json["suggestion_text"].ToString());

                            //Data Logs
                            sp_register_data_logs(json["CRN"].ToString(), 0, 0, 0, "{\"Data\":\"" + json.ToString() + "\"}", "", "Calling Survey User Answer API");

                            int status_code = InsertSurveyAnswers(json, get_user_survey_id);
                        //string result = "{\"StatusCode\": \"SURVEY200\"}";
                        string result = SurveyAnswerResponseHashing("SURVEY200", SequenceId);
                        sp_register_data_logs(json["CRN"].ToString(), 0, 0, 0, "{\"Data\":\"" + result + "\"}", "", "Response Survey User Answer API");
                    ...
                    else
                    {
                        ...
                        //Data Logs
                        sp_register_data_logs(CRN, 0, 0, 0, "", "{\"Response\":\"SURVEY101\",\"Message\":'Insufficient Headers in Survey Validatation (CRN, SequenceId and Authorization)'}", "Response Survey Validation API");

                        throw new InsufficientHeaderDetails("SURVEY101", "Insufficient Headers in Survey Validatation (CRN, SequenceId and Authorization)");
                    }
                }
        ...
        public int InsertSurveyAnswers(JObject json, string user_survey_id)
        {
            using (var connection = new OracleConnection(DBConnection))
            {
                connection.Open();
                JArray ja = (JArray)json["answer_data"];
                string[] user_survey_ids = new string[ja.Count];
        ...
        private bool SurveyAnswerRequestHashing(string CRN, string SequenceId, string Signature)
        {
            return Signature.Equals(SurveyGetHash(SequenceId.Substring(3, 4) + CRN + SequenceId.Substring(11, 4)), StringComparison.Ordinal);
        }

        private string SurveyAnswerResponseHashing(string Code, string SequenceId)
        {
            return SurveyGetHash(SequenceId.Substring(16, 4) + Code + SequenceId.Substring(22, 4));
        }
        ...
        private string SurveyGetHash(string Text)
        {
            HMACSHA256 hmac = new HMACSHA256(SurveySecretKey);
            hmac.Initialize();
            byte[] bytes = Encoding.UTF8.GetBytes(Text);
            byte[] rawHmac = hmac.ComputeHash(bytes);
            return Convert.ToBase64String(rawHmac);
