...
namespace KotakFacialAuthenticationRestWebService.Controllers
{
    public class KotakAPIController : ApiController
    {
        ...
        private string subscriptionKey = ConfigurationManager.AppSettings["FaceAPIKey"], FaceIDEndpoint = ConfigurationManager.AppSettings["FaceAPIEndPoint"], PersonGroupId = ConfigurationManager.AppSettings["PersonGroupId"], DataMigratePersonGroupId = ConfigurationManager.AppSettings["DataMigratePersonGroupId"];
            ...
            try
            {
                ...
                for (int i=0;i<GetRegisteredUserData.Count;i++)
                {
                    ...
                    if (result.StatusId == 200)
                    {
                        ...
                        customer_registration_checker_id[i] = GetRegisteredUserData[i].customer_registration_checker_id;
                    }
                }

                result = AzureAPI.TrainPersonGroup();

                //es.update_resource_group_id(DataMigratePersonGroupId, customer_registration_checker_id.ToString());
                return output;
            }
            catch(Exception e)
            {
