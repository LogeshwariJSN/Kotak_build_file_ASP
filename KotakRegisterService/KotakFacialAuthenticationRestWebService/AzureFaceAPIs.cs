...
namespace KMBL.StepupAuthentication.CoreComponents
{
    public class AzureFaceAPIs
    {
        ...
        public AzureFaceAPIs(string FaceIDEndpoint,string PersonGroupId, string subscriptionKey, string ProxyServerUrl=null, string ProxyFlag="0")
        {
        ...
        public AzureFaceAPIs DetectFace(byte[] imageBytes)
        {
            ...
            if ((int)response.StatusCode == 200)
            {
                ...
                if(result.Count == 1)
                    ...
                    return new AzureFaceAPIs { StatusId = 201, Message = "No Face Found" };
                else
                    return new AzureFaceAPIs { StatusId = 202, Message = "Multiple Face Found" };
            }
        ...
        public AzureFaceAPIs IdentifyPerson(string FaceID)
        {
            ...
            if ((int)response.StatusCode == 200)
            {
                ...
                if(result.Count==1 && result[0].candidates.Count==1)
                    return new AzureFaceAPIs { StatusId = 200, Message = result[0].candidates[0].personId };
                else
