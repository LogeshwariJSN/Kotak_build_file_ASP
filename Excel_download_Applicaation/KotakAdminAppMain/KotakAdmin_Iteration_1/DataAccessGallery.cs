...
namespace KotakAdmin_Iteration_1
{
    ...
    public class DataAccessGallery
    {
        ...
        public List<UserImageZipDownload> UserImageZipDownload(string crn, string event_id, string result_reason, string gate_number, string from_date, string to_date, string version)
        {
            ...
            while (dataReader.Read())
            {
                var ImageGallery = new ImageGallery(MongoDBConnectionstring, (Convert.ToInt32(event_id) == 1) ? MongoDB_Registration_Databse : MongoDB_Verification_Databse);
                ...
                uizd.customer_images = dataReader.IsDBNull(2) ? null : Convert.ToBase64String(ImageGallery.Get(dataReader.GetString(3)));
                aled.Add(uizd);
            }
            dataReader.Close();
            dbManager.CloseConnection(connection);
