                            ...
                            if ((Number(JSON.parse(send_data).offset_value) + Number(JSON.parse(send_data).record_limit)) >= response.Data.total_records) {
                                ...
                                if (Number(JSON.parse(send_data).offset_value) == 0) {
                                    ...
                                    if (button_id == 4) {
                                        ...
                                        if (Number($('#top_ending_index').html()) == Number($('#top_total_count').html())) {
                                            $('.next_btn').attr("disabled", true);
                                            $('.last_btn').attr("disabled", true);
                                        }
                                        else {
...
function audit_log_register_attempt() {
    ...
    $.ajax({
        ...
        datatype: "json",
        success: function (response) {
            var registration = 0, verification = 0;

            if (JSON.parse(send_data).event_id == 1) {
                //$('#registration_div').show();
                //$('#verification_div').hide();
                registration = 1;

            } else if (JSON.parse(send_data).event_id == 2) {
            ...
            if (registration == 1) {
                ...
                $('#RegAverageDroppedUsers').html(response.RegData.reg_average_dropped_users == 'null' ? 0 : response.RegData.reg_average_dropped_users);
                ...
                $('#RegCompleteSingleAttempt').html(0);
                $('#RegCompleteMultipleAttempt').html(0);
                $('#RegAverageCompleteUsers').html(0);
                $('#RegDropSingleAttempt').html(0);
                $('#RegDropMultipleAttempt').html(0);
                $('#RegAverageDroppedUsers').html(0);
                $('#RegSuccessfulAttempts').html(0);
                $('#RegFailureAttempts').html(0);
            }
            if (verification == 1) {
                ...
                $('#VerifyAverageDroppedUsers').html(response.VerifyData.verify_average_dropped_users == 'null' ? 0 : response.RegData.verify_average_dropped_users);
                ...
                $("#RegVerifyDone").html(Number(response.VerifyData.verify_successful_attempts));
            }
            else {
                $('#VerifyCompleteSingleAttempt').html(0);
                $('#VerifyCompleteMultipleAttempt').html(0);
                $('#VerifyAverageCompleteUsers').html(0);
                $('#VerifyDropSingleAttempt').html(0);
                $('#VerifyDropMultipleAttempt').html(0);
                $('#VerifyAverageDroppedUsers').html(0);
                $('#VerifySuccessfulAttempts').html(0);
                $('#VerifyFailureAttempts').html(0);

                //$("#RegVerifyDone").html(Number(response.VerifyData.verify_successful_attempts) + Number(response.VerifyData.verify_failed_attempts));
                $("#RegVerifyDone").html(Number(0));
            }
        },
        error: function (response) {
...
function user_image_zip_download() {
    ...
    $.ajax({
        ...
        success: function (response) {
            console.log(response);   
            if (response.StatusCode == 200) {
                if (response.UserImageZipDownloadData.length >0) {
                    var images_arr = [], crn_arr = [], created_on_arr = [];
                    for (var i = 0; i < response.UserImageZipDownloadData.length; i++) {
                        if (response.UserImageZipDownloadData[i].customer_images != "") {
                            var image_base64 = "data:image/png;base64," + response.UserImageZipDownloadData[i].customer_images;
                            images_arr.push(image_base64);
                            var CreatedOn = response.UserImageZipDownloadData[i].CreatedOn.replaceAll(':','-');
                            created_on_arr.push(CreatedOn);
                            var CRN = response.UserImageZipDownloadData[i].CRN;
                            crn_arr.push(CRN);
                        }
                    }


                    var filename = "Audit_Log_"+JSON.parse(send_data).from_date + " to " + JSON.parse(send_data).to_date;
                    //The function is called
                    compressed_img(images_arr, filename, crn_arr, created_on_arr);
                }
