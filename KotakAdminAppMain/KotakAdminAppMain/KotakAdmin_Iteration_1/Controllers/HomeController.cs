        ...
        public JsonResult data_migration_on_start(int customer_register_checker_id)
        {
            try
            {
                ...
                if (result.StatusId == 200)
                {
                    ...
                    for (int j = 0; j < 2; j++)
                    {
                        result = AzureAPI.AddFace(Convert.FromBase64String(GetCustomerImageData.customer_image_id), PersonID); // Calling AddFace Function
                    }
                    DataAccessGallery.update_resource_group_id(DataMigratePersonGroupId, PersonID, GetCustomerImageData.customer_registration_checker_id);
                }
                else
                {
