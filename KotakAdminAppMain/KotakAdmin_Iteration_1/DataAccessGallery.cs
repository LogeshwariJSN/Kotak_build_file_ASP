...
namespace KotakAdmin_Iteration_1
{
    ...
    public class DataAccessGallery
    {
        ...
        // Log Function
        public static JsonResult AuditLogFetch(string crn, int event_id, string result_reason, int gate_number, string from_date, string to_date, string version, int offset_value, int no_of_records)
        {
            int total_records = 0;
            ...
            //parameters.Add(dbManager.CreateParameter("v_offset", 0, OracleDbType.Int32));
            //parameters.Add(dbManager.CreateParameter("v_no_of_records", 10, OracleDbType.Int32));
            //parameters.Add(dbManager.CreateParameter("v_crn", "xxx", OracleDbType.Varchar2));
            //parameters.Add(dbManager.CreateParameter("v_event_id", 1, OracleDbType.Int32));
            //parameters.Add(dbManager.CreateParameter("v_gate_number", 2, OracleDbType.Int32));
            //parameters.Add(dbManager.CreateParameter("v_from_date", "", OracleDbType.Varchar2));
            //parameters.Add(dbManager.CreateParameter("v_to_date", "", OracleDbType.Varchar2));
            //parameters.Add(dbManager.CreateParameter("v_version", 8, OracleDbType.Varchar2));
            //parameters.Add(dbManager.CreateParameter("l_cursor", OracleDbType.RefCursor, ParameterDirection.Output));
            //parameters.Add(dbManager.CreateParameter("v_total_records", OracleDbType.Int32, ParameterDirection.Output));
            ...
            while (reader.Read())
            {
                string result_reason_get = "";
                if (!reader.IsDBNull(16))
                {
                    if (reader.GetString(16) == "Azure Registration Failed" || reader.GetString(16) == "Azure Registration Succeeded" || reader.GetString(16) == "Azure Verification Failed" || reader.GetString(16) == "Azure Verification Succeeded")
                    {
                        result_reason_get = reader.GetString(16);
                    }
                    else
                    {
                        JObject result_reason_json = JObject.Parse(reader.GetString(16));

                        string gate_1_get_result = result_reason_json["KotakComputerVisionGate1"]["Gatename"]["ResponseFromGate"]["Result"].ToString();
                        if (gate_1_get_result == "Invalid Angle. Please look directly in the camera with neutral expression and open eyes")
                        {
                            result_reason_get = "Invalid Angle";
                        }
                        else if (gate_1_get_result == "Ensure you position only your face in the mobile frame")
                        {
                            result_reason_get = "Invalid Face";
                        }
                        else if (gate_1_get_result == "Please remove your glassess and try again")
                        {
                            result_reason_get = "Sunglass";
                        }

                        string gate_2_get_result = result_reason_json["KotakComputerVisionGate2"]["Gatename"]["ResponseFromGate"]["Result"].ToString();
                        if (gate_2_get_result != "Sucesss")
                        {
                            result_reason_get = "Fake";
                        }

                        string gate_3_get_result = result_reason_json["KotakComputerVisionGate3"]["Gatename"]["ResponseFromGate"]["Result"].ToString();
                        if (gate_3_get_result != "Sucesss")
                        {
                            result_reason_get = "Not Smile";
                        }
                ...
                ad.Add(new KMBLNetNbanking.AuditLogFetch
                {
                    ...
                    //CreatedOn = reader.IsDBNull(2) ? null : reader.GetString(2),
                    EventName = reader.IsDBNull(3) ? "" : reader.GetString(3),
                    ...
                    GateFailedAt = reader.IsDBNull(14) ? "" : reader.GetString(14),
                    ParentTransactionID = reader.GetInt32(9),
                    ChannelName = reader.IsDBNull(10) ? "" : reader.GetString(10),
                    Score = reader.IsDBNull(12) ? 0 : reader.GetInt32(12),
                    IsCompleted = reader.GetInt32(13),
                    Version = reader.IsDBNull(15) ? "1" : reader.GetString(15),
                    json_response = reader.IsDBNull(16) ? "" : reader.GetString(16),
                    ResultReason = result_reason_get,
                    AuditLogExpandFetch = AuditLogExpandFetch(reader.GetInt32(0), reader.GetString(3))
                });
            }
            reader.Close();
            dbManager.CloseConnection(connection);
            total_records = Convert.ToInt32(objCmd.Parameters["v_total_records"].Value);
        ...
        public static List<KMBLNetNbanking.AuditLogExpandFetch> AuditLogExpandFetch(int audit_id, string event_name)
        {
            ...
            while (reader.Read())
            {
                ...
                aed.Add(new KMBLNetNbanking.AuditLogExpandFetch
                {
                    ...
                    Image_get = reader.IsDBNull(1)? "../../assets/images/empty_person.png" : "data:image / jpeg; base64," + Convert.ToBase64String(img.Get(reader.GetString(1))),
                    GATE_NAME = reader.IsDBNull(2)?0: reader.GetInt32(2),
                    threshold_value = reader.IsDBNull(3)?"": reader.GetString(3),
                    STATUS_NAME = reader.IsDBNull(4)?"": reader.GetString(4),
                    JSON_RESPONSE = reader.IsDBNull(5)?"": reader.GetString(5),
                });
            }
            reader.Close();
            dbManager.CloseConnection(connection);
        ...
        public AuditLogRegisterCount AuditLogRegisterCount(string crn, int event_id, string result_reason, int gate_number, string from_date, string to_date, string version)
        {
            ...
            alr1.reg_average_dropped_users= Convert.IsDBNull(objCmd.Parameters["reg_average_dropped_users"].Value) ? 0 : Convert.ToInt32(objCmd.Parameters["reg_average_dropped_users"].Value.ToString());
        ...
        public AuditLogVerifyCount AuditLogVerifyCount(string crn, int event_id, string result_reason, int gate_number, string from_date, string to_date, string version)
        {
            ...
            alv1.verify_average_completed_users= Convert.ToInt32(objCmd.Parameters["verify_average_completed_users"].Value.ToString());
            alv1.verify_drop_single_attempt = Convert.ToInt32(objCmd.Parameters["verify_drop_single_attempt"].Value.ToString());
            alv1.verify_drop_multiple_attempt = Convert.ToInt32(objCmd.Parameters["verify_drop_multiple_attempt"].Value.ToString());
            alv1.verify_average_dropped_users= Convert.ToInt32(objCmd.Parameters["verify_average_dropped_users"].Value.ToString());
            alv1.verify_successful_attempts = Convert.ToInt32(objCmd.Parameters["verify_successful_attempts"].Value.ToString());
            alv1.verify_failed_attempts = Convert.ToInt32(objCmd.Parameters["verify_failed_attempts"].Value.ToString());
