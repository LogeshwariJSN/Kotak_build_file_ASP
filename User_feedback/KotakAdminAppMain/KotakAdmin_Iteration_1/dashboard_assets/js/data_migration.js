                    ...
                    if (response.Data.length == 0) {
                        ...
                        for (var i = 0; i < response.Data.length; i++) {
                            if (response.Data[i].trained_resource_group_id == "001") {
                                ...
                                current_resource_tr_html += '<tr id="current_resource_tr' + response.Data[i].customer_registration_checker_id + '"><td id="crn_td' + response.Data[i].customer_registration_checker_id + '">' + response.Data[i].CRN + '</td> <td id="registered_at_td' + response.Data[i].customer_registration_checker_id + '">' + response.Data[i].created_on + '</td><td><button type="button" class="btn btn-primary move_btn" id="move_btn' + response.Data[i].customer_registration_checker_id + '" customer_registration_checker_id ="' + response.Data[i].customer_registration_checker_id + '">Move</button></td></tr>';
...
$(document).on('click', '.move_btn', function () {
    ...
    var customer_registration_checker_id = $(this).attr('customer_registration_checker_id');
    ...
    var JsonData = JSON.stringify({ customer_register_checker_id: customer_registration_checker_id });
    $.ajax({
        type: "POST",
        data: JsonData,
        ...
        success: function (response) {
            ...
            try {
                ...
                //};
                if (response.StatusCode == 200) {
                    $('#move_btn' + JSON.parse(JsonData).customer_register_checker_id).html("Move");
                    $('.move_btn').attr("disabled", false);

                    var get_crn = $('#crn_td' + JSON.parse(JsonData).customer_register_checker_id).html();
                    var get_registered_at = $('#registered_at_td' + JSON.parse(JsonData).customer_register_checker_id).html();
                    $('#new_resource_table tbody').append("<tr><td>" + get_crn + "</td><td>" + get_registered_at + "</td></tr>");

                    $('#current_resource_tr' + JSON.parse(JsonData).customer_register_checker_id).remove();
